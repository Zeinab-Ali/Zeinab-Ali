...
if ($_GET) {
    if (isset($_GET['subcategory'])) {
        if (is_numeric($_GET['subcategory'])) {
            $sub = new Subcategory;
            $sub->setId($_GET['subcategory']);
            $subResult = $sub->find()->get_result();
            if ($subResult->num_rows == 1) {
                $pro->setSubcategory_id($_GET['subcategory']);
                $proResult = $pro->getProductsBySub()->get_result();
            } else {
        ...
        if (is_numeric($_GET['brand'])) {
            ...
            $brandResult = $brand->find()->get_result();
            if ($brandResult->num_rows == 1) {
                $pro->setBrand_id($_GET['brand']);
                $proResult = $pro->getProductsByBrand()->get_result();
            } else {
                include "layouts/errors/404-not-found.php";
                die;
            }
        ...
        if (is_numeric($_GET['category'])) {
            $cat = new Category;
            $cat->setId($_GET['category']);
            $catResult = $cat->find()->get_result();
            if ($catResult->num_rows == 1) {
                $pro->category_id = $_GET['category'];
                $proResult = $pro->getProductsByCategory()->get_result();
            } else {
                include "layouts/errors/404-not-found.php";
                die;
            }
                    ...
                    <div class="product-grid product-view pb-20">
                        <div class="row">
                            ...
                            if ($proResult->num_rows >= 1) {
                                foreach ($proResult->fetch_all(MYSQLI_ASSOC) as $product) { ?>
